...
Page({
    ...
    async obtainPostListPage(pageNum = 1, num = 5) {
        if (!this.data.isLoading) {
            ...
            let ares = await DB.collection("posts").where({
                context: DB.RegExp({
                    regexp: this.data.keyword,
                    options: 'i',
                }),
            ...
            if (index == -1) {
                ...
                wx.setStorageSync('historySearchPostList', JSON.stringify(this.data.historys));
            }
        }
    },
    //跳转
    index(e) {
        wx.navigateTo({
            url: '../post/post?id=' + e.currentTarget.dataset.index,
        })
    },
    clearHistory() {
        this.setData({
        ...
        wx.setStorageSync('historySearchPostList', '[]');
    },
    ...
    onLoad(options) {
        this.setData({
            'historys': JSON.parse(wx.getStorageSync('historySearchPostList') || '[]')
        });
        if (this.data.historys.length != 0) {
            this.setData({
                aaa: 1,
    ...
    onReachBottom: function () {
        if (this.data.pageNum < this.data.pages) {
            ...
            this.obtainPostListPage(this.data.pageNum);
